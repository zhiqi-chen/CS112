...
public class LinkedTree {
    ...
    public void insert(int key, Object data) {
        ...
        if (parent_node == null) {    // the tree was empty
            root = newNode;
        } else if (key < parent_node.key) {
            parent_node.left = newNode;             
            newNode.parent=parent_node;                //set the parent refference
        } else {
    ...
    private class InorderIterator implements LinkedTreeIterator {
        ...
        private InorderIterator() {
            ...
            while(nextNode.left!=null){         // find the left most child
                nextNode=nextNode.left;
            }
        }
        ...
        public int next() {
            ...
            int key = nextNode.key;                         
            if (nextNode.right != null) {               //check if it has right child
                nextNode = nextNode.right;              //if have, go to the left most child of hte right child
                while (nextNode.left != null) nextNode = nextNode.left;
            } else {                                       // if not right child, go back to it's parent
                while (nextNode.parent != null && nextNode == nextNode.parent.right) nextNode = nextNode.parent;
    ...
    public static void main(String[] args) {
         ...
         */  
        System.out.println("--- Testing Problem 6 ---");                //testing
        System.out.println();
        System.out.println("(0) Testing on Problem 6");
        try {
        LinkedTree tree = new LinkedTree();
        int[] keys = {44, 35, 53, 23, 48, 62, 28, 57, 80};
        tree.insertKeys(keys);
        ...
        System.out.println(results);    
        System.out.println("expected results:");
        ArrayList<Integer> answer = new ArrayList<Integer>(9);
        answer.add(23);
        answer.add(28);
        answer.add(35);
        answer.add(44);
        ...
        System.out.print("MATCHES EXPECTED RESULTS?: "); 
        System.out.println(results.equals(answer));
        } catch (Exception e) {
            System.out.println("INCORRECTLY THREW AN EXCEPTION: " + e);
        }
    }

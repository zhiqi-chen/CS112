/*
    Student: Zhiqi Chen
    Added function: removeAll: remove all of the input item in the LLList, and return true if at least one 
...
public class LLList implements List {
    ...
    public boolean removeAll(Object item){
        boolean result=false;               //set the initial result to false, and if at least one item is removed from the LLList, then change result to true
        for (int i=0;i<length;i++){         
            if(getItem(i).equals(item)){    // go through the LLList, use getItem to get the item at the position i of the list  and compare with the input item
                removeItem(i);              // use removeItem to remove the item from the LLList if found that it is same as the input item
                result=true;                
            }
        }                                   //O(n)running time
        return result;
